...
var _ = Describe("test kvm export/import integration", func() {
	...
	BeforeEach(func() {
		...
		err := setup.SetupKVM(ctx, k8sClient)
		Expect(err).ToNot(HaveOccurred())
	})

	It("reconcile object status", func() {
		if !useExisting {
			Skip("skipping kvm integration tests as not using an existing environment")
		}

		By("checking if kvm migration is ready", func() {
			Eventually(func() error {
				v := &migration.KVMSource{}
				err := k8sClient.Get(ctx, setup.KVMSourceNamespacedName, v)
				if err != nil {
					return err
				}
		...
		By("vm importjob has the correct conditions", func() {
			Eventually(func() error {
				...
				err := k8sClient.Get(ctx, setup.KVMVMNamespacedName, v)
				if err != nil {
					return err
				}
		...
		By("checking that the virtualmachine has been created", func() {
			Eventually(func() error {
				...
				err = k8sClient.Get(ctx, types.NamespacedName{
					Namespace: setup.KVMVMNamespacedName.Namespace,
	...
	AfterEach(func() {
		...
		err := setup.CleanupKVM(ctx, k8sClient)
		Expect(err).ToNot(HaveOccurred())
	})
})
