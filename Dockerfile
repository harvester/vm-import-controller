...
RUN zypper -n rm container-suseconnect 2>/dev/null || true && \
    zypper -n install tar gzip bash git gcc docker curl wget ca-certificates && \
...
ENV HOME=/go/src/github.com/harvester/vm-import-controller


# ---- base ----
FROM builder AS base
WORKDIR /go/src/github.com/harvester/vm-import-controller
...
RUN --mount=type=cache,target=/go/pkg/mod,id=harvester-go-mod-${MK_REPO_ID} \
    --mount=type=cache,target=/go/src/github.com/harvester/vm-import-controller/.cache/go-build,id=harvester-go-build-${MK_REPO_ID} \
    cp /go/src/github.com/harvester/vm-import-controller/package/qemu-wrapper.sh /usr/bin/ && \
    ./scripts/build

FROM scratch AS build-output
COPY --from=build /go/src/github.com/harvester/vm-import-controller/bin/ /bin/
