...
// VmwareSourceController interface for managing VmwareSource resources.
type VmwareSourceController interface {
	generic.ControllerInterface[*v1beta1.VmwareSource, *v1beta1.VmwareSourceList]
}

// VmwareSourceClient interface for managing VmwareSource resources in Kubernetes.
type VmwareSourceClient interface {
	generic.ClientInterface[*v1beta1.VmwareSource, *v1beta1.VmwareSourceList]
}

// VmwareSourceCache interface for retrieving VmwareSource resources in memory.
type VmwareSourceCache interface {
	generic.CacheInterface[*v1beta1.VmwareSource]
}

// VmwareSourceStatusHandler is executed for every added or modified VmwareSource. Should return the new status to be updated
...
// VmwareSourceGeneratingHandler is the top-level handler that is executed for every VmwareSource event. It extends VmwareSourceStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// RegisterVmwareSourceStatusHandler configures a VmwareSourceController to execute a VmwareSourceStatusHandler for every events observed.
...
// RegisterVmwareSourceGeneratingHandler configures a VmwareSourceController to execute a VmwareSourceGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured VmwareSourceGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *vmwareSourceGeneratingHandler) isNewResourceVersion(obj *v1beta1.VmwareSource) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *vmwareSourceGeneratingHandler) storeResourceVersion(obj *v1beta1.VmwareSource) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
