...
func (h *virtualMachineHandler) findAndCreatePVC(vm *migration.VirtualMachineImport) error {
	for _, v := range vm.Status.DiskImportStatus {
		...
		// vmiBackend, err := util.GetBackendFromStorageClassName(vmiObj.Status.StorageClassName, h.sc)
		// if err != nil {
		// 	return fmt.Errorf("failed to get VMI backend from storage class '%s': %v", vmiObj.Status.StorageClassName, err)
		// }
		// if vmiBackend == harvesterv1beta1.VMIBackendCDI {
		// 	createPVC = false // CDI backend does not require PVC creation
		// }
